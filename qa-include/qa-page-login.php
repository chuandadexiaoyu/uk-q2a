...
//				print_r($uid);
				print_r("<br>这是$ucsyslogin:<br>");
				print_r($ucsynlogin);s
				exit("<br>进入discuz成功了");
				//以上是同步登陆discuz
				$matchusers=qa_db_user_find_by_handle($inemailhandle);
				if(count($matchusers)!=1)//不等于一就默认没有用户
				{
					$userid=qa_create_new_user($email, $password, $username);//创建用户
				}
				$inuserid=$matchusers[0];
				$userinfo=qa_db_select_with_pending(qa_db_user_account_selectspec($inuserid, true));
				require_once QA_INCLUDE_DIR.'qa-app-users.php';
				qa_set_logged_in_user($inuserid, $userinfo['handle'], $inremember ? true : false);//登陆uk-q2a
				$topath=qa_get('to');
				if (isset($topath))
				{
