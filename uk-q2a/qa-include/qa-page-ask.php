...
//	Check whether this is a follow-on question and get some info we need from the database
	...
	$in['categoryid']=qa_get_category_field_value('category');  //  User entered categoryid
...
//	Process input   Process input   Process input   Process input    Process input    
...
if($tag_length==0)
{
$errors['tags']="标签不能为空哦，亲";
}
else
{
for($i=0;$i<$tag_length;$i++)
{
if($in['tags'][$i]==$tag_start[$i])
{
$errors['tags']=$tag_start[$i];
break;
}
}
}
...
//	Prepare content for theme   Prepare content for theme   Prepare content for theme
...
//Hide the common problem category
...
array_splice($qa_content['form']['fields'],3,0,array('tags' => $field));
