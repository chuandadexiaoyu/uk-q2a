...
//ADD
function uc_get_user_username($username, $isuid=0) {
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user', array('username'=>$username, 'isuid'=>$isuid));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}

//ADD
function uc_get_user_email($email, $isuid=0) {
print("I WILL USE GET BY EMAIL<br>");
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user', array('email'=>$email, 'isuid'=>$isuid));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
