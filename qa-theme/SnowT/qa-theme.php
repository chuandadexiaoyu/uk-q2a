...
//echo "<script type='text/javascript' language='javascript' src='./qa-theme/SnowT/js/home_store.js'></script>";
	...
	*/
      ...
      $this->output('<SCRIPT TYPE="text/javascript" src="./qa-content/jquery.tagsinput.js"></SCRIPT>');
$this->output('<script type=\'text/javascript\' src=\'http://xoxco.com/x/tagsinput/jquery-autocomplete/jquery.autocomplete.min.js\'></script>');
			$this->output(
		...
		function nav_user_search() // outputs login form if user not logged in
		{
			if(!qa_is_logged_in()){
				...
				if(isset($login)){
						...
						'<td><a class="tooltip01" href="http://210.209.123.136/discuz/forum.php" style=" font-size:12px;text-decoration:underline;">找回密码</a></td>',
						...
						'<td><a href="http://210.209.123.136/discuz/member.php?mod=register" style=" font-size:12px;text-decoration:underline" class="tooltip02" ><center>注册</center></a></td>',
						'</tr>',
/*
						'<tr>',
						'<td></td>',
						'<td><p class="tooltip02" style="" >帐号小提示</p></td>',
						'</tr>',
*/
						'</tbody>',						
						'</table>',
...
function search()
{
	...
	$this->output('<FORM METHOD="GET" ACTION="./index.php?qa=search">');
  $this->output('<INPUT TYPE="hidden" NAME="qa" VALUE="search">');
	$this->output('<INPUT TYPE="text" NAME="q" VALUE="" CLASS="qa-search-field">');
	$this->output('<INPUT TYPE="submit" VALUE="搜索" CLASS="qa-search-button">');
	$this->output('</FORM>');
	$this->output('</DIV>');
}
...
{
$announcement=$this->content['announcement'];
...
{
	$link="http://210.209.123.136/uk-q2a/index.php?qa=".$announ['tags'];
	$this->output('<LI id="f_a"><A href="'.$link.'">'.$announ['heading'].'</A></LI>');
}
...
{
$problemcategory=$this->content['ukpopulartags'];
$this->output('<DIV class="qa_ukpc" style="">');
$this->output('<H4 class="left_h4 problem_h4" ><span class="h4_img"><img src="./qa-theme/SnowT/images/z_h4_lifront.jpg"/></span><A HREF="./index.php?qa=tags">火热标签</A></H4>');
$this->output('<br><div class="qa_ukpc1">');
$i=1;
foreach($problemcategory as $key=>$value)
{
$this->output('<img src="./qa-theme/SnowT/images/z_h4_lifront.jpg"/><A HREF="./index.php?qa=tag&qa_1='.$key.'">');
$this->output($key);
$this->output('</A>');
$this->output('<br>');
$i++;
...
{
	$jiaocheng=$this->content['jiaocheng'];
...
foreach($jiaocheng as $jc)
{
	$link="http://210.209.123.136/uk-q2a/index.php?qa=".$jc['tags'];
	$pic_link="./qa-theme/SnowT/images/".$jc['tags'];
	$this->output('<A href="'.$link.'" class="c_img"><img src="'.$pic_link.'.png"></img><P>'.$jc['heading'].'</P></A>');
}

/*
$this->output('<A href="#" class="c_img" id="f_cc"><img src="./qa-theme/SnowT/images/jc_1.png"></img><P>系统安装</P></A>');
