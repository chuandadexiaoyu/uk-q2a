...
function qa_tag_click(link)
{
	...
	if (qa_tag_onlycomma)//下面的replace的* /之间的空格应该消失
...
{
qa_start();
var class_y=link.className.split(" ")[0];
var class_x=link.className.split(" ")[1];//类名

if(class_y=="qa-tag-input")
{
var qa_value=link.value;//值
switch (class_x)
{
case "qa-version":qa_change_string(0,qa_value);break;
case "qa-shop":qa_change_string(1,qa_value);break;
default:;
}
qa_change_bg_input(link.id,class_x);
}
...
{
var qa_value=link.firstChild.innerHTML;//值
switch (class_x)
{
case "qa-version":qa_change_string(0,qa_value);document.getElementById("version05").value="";break;
case "qa-shop":qa_change_string(1,qa_value);document.getElementById("shop05").value="";break;
default:;
}



qa_change_bg_a(link.id,class_x);
...
function qa_change_bg_a(id,class_x)
{
var categories=document.getElementsByClassName(class_x);//一个数组
...
{
categories[i].className="qa-tag-link "+class_x;
}
document.getElementById(id).className += " qa-tag-link-another";

}


function qa_change_bg_input(id,class_x)
{
var categories=document.getElementsByClassName(class_x);//一个数组
...
function qa_start()
{
var qa_tag_input=document.getElementById("tags").value;
if(qa_tag_input == "")
{
string_version="版本没有输入";
string_shop="厂商没有输入";
string_any="";
document.getElementById("tags").value=string_version+","+string_shop+","+string_any;
}
}


function qa_any_click(link)
{
qa_start();
var qa_value=document.getElementById(link.id).value;
qa_change_string(2,qa_value);
}


function qa_change_string(place,string_value)
{
var qa_tag_input=(document.getElementById("tags")).value;
var word=qa_tag_input.split(",");
word.splice(place,1,string_value);
(document.getElementById("tags")).value=word[0]+","+word[1]+","+word[2];
}
